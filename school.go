...
func SkillChecker(w http.ResponseWriter, r *http.Request) {
	history, ok := r.Context().Value("history").(model.History)
	if !ok {
		w.WriteHeader(500)
		return
	...
	if err != nil {
		write(w, fmt.Sprintf(simpleFormat, "죄송해요, 잘 못 들었어요."))
		return
	}
	...
	write(w, fmt.Sprintf(simpleFormat, "취소가 완료되었습니다."))
}
...
func SkillEnterStudentInfo(w http.ResponseWriter, r *http.Request) {
	...
	if history.User.StudentID != 0 {
		write(w, fmt.Sprintf(simpleFormatWithoutRetry, "이미 내 정보가 등록되어 있습니다. 이 정보는 바꿀 수 없습니다."))
		return
	}
	...
	if err != nil {
		write(w, fmt.Sprintf(simpleFormat, "유효한 학생 정보가 아닙니다. 올바르게 입력했는지 확인해주세요. 번호를 입력 시 '23번'이 아닌 '23'으로 입력해야합니다.", BlockID))
		return
	}
	...
	err = s.Find(db)
	if err != nil {
		write(w, fmt.Sprintf(simpleFormat, "유효한 학생 정보가 아닙니다. 올바르게 입력했는지 확인해주세요.", BlockID))
		return
	}
	...
	write(w, fmt.Sprintf(simpleFormatWithoutRetry, "성공적으로 내 정보를 등록하였습니다."))
}
